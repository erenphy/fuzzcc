...
import globalVar
import ccmounter
#多进程执行，进行种子生成，种子变异和执行
#以此实现共享testcases
#  ccgenerator.py---->种子生成
#  ccmutator.py----->种子变异
#  ccmounter.py----->取测试用例，并执行挂载。需要增加一个收集执行完状态＋比较环节
#  usage: python3 ccserver.py -k (is_kernel_fs) -T (target img) -t (fs-type) -o output
...
def p_mutator(seedq, testcaseq):
	...
	while True:
		...
		seedq.task_done()
		testcaseq.join()
def p_fuzzer(testcaseq, iskfs, fstype, targetimg, output, timeout):
	print("----------------------fuzzing by" + str(os.getpid()) + '\n')
	# setproctitle.setproctitle("p_fuzzer")
	while True:
		a = testcaseq.get()
		# if a == None: break
		my_mntpoint = ccmounter.cc_mounter(iskfs, fstype, targetimg, a, output)
		# ccparser.cc_parser(my_mntpoint, a)
		print(my_mntpoint)
		testcaseq.task_done()
...
if __name__ == '__main__':
	...
	parser.add_argument('-o', dest='output', help='path to save the converted file')

	args = parser.parse_args()

	if args.targetimg is None or args.fs_type is None or args.output is None or not args.is_kernel_fs in ['0', '1'] :
	...
	fuzzer_pro = Process(target = p_fuzzer, args = (globalVar.TESTCASE_QUEUE, iskfs, fstype, targetimg, output, outoftime), daemon= True)
